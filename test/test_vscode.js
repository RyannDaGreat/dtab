...
async function main() {
    ...
    const {selectedLines, lineEdit} = require(path.join(EXTENSION, manifest.main))
    assert.deepStrictEqual(selectedLines({start: {line: 1, character: 0}, end: {line: 3, character: 0}}), [1, 2], 'a selection ending at column 0 stops on the line before')
    assert.deepStrictEqual(selectedLines({start: {line: 1, character: 2}, end: {line: 1, character: 5}}), [1, 1])
    assert.deepStrictEqual(selectedLines({start: {line: 4, character: 0}, end: {line: 4, character: 0}}), [4, 4], 'an empty selection is its own line')
    assert.deepStrictEqual(lineEdit('\tdef f():', '\t    def f():'), {at: 1, insert: '    '})
    assert.deepStrictEqual(lineEdit('\t    return', '\treturn'), {from: 1, to: 5})
    assert.deepStrictEqual(lineEdit('a\tb 1', '\ta\tb 1'), {at: 0, insert: '\t'})
    assert.deepStrictEqual(lineEdit('\ta', 'a'), {from: 0, to: 1})
    assert.strictEqual(lineEdit('a', 'a'), null)
    assert.ok(manifest.contributes.keybindings.some(k => k.key === 'shift+tab' && k.when.includes('!editorTabMovesFocus')), 'Shift+Tab must honour tab-moves-focus mode like Tab')
