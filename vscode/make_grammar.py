...
# A block rule's begin match ends right after `$key [tag]`, so the rest of that line is block content like
# every deeper line: `$cmd bash<Tab>echo hi` is highlighted as bash from `echo` on, as the parser reads it.
BLOCK_CAPTURES = {
...
def block_rule(tag_regex, scope, name):
    """Pure function. The rule for `$key TAG`: the block's text goes to that language's grammar."""
    return {
...
def grammar():
    ...
    return {
        ...
        "comment": "Same tokens as dtab.vim: an entry between tabs is a comment (leading space), a leaf (key value) or an "
                   ...
                   "block: the rest of that line and every following line indented deeper than the $ line (bound with a "
                   "backreference to its tabs) is value text, highlighted as the tag's language, or as the language named by "
                   "a shebang as the block's first text. Generated by make_grammar.py; do not edit by hand.",
        "patterns": [
            {"comment": "A whitespace-only line", "match": "^\\s*$"},
        ] + [block_rule(*e) for e in EMBEDDED] + [
            {
                "comment": "$key with no language tag: plain block text, unless its first text is a shebang",
                "begin": "^(\\t*)((?:[^\\t\\n]*\\t+)*)(\\$%s)(?: ([^\\t]*))?(?=\\t|$)" % KEYS,
                "beginCaptures": BLOCK_CAPTURES,
                "while": WHILE,
