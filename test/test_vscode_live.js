// Runs the dtab extension inside the installed VS Code (a throwaway profile in a temp directory, no
...
async function run() {
    ...
    // The header line `q sql` is painted by semantic tokens (the grammar cannot see the deeper line): key and tag, 5 ints each.
    ...
    let r = await run('x\n\tcode \n\t\tbody\n\t\n', [S(P(3, 1))], 'dtab.tab')
    assert.strictEqual(r.text, 'x\n\tcode \n\t\tbody\n\t\t\n', 'Tab on a whitespace line at the header depth: exactly one tab, not a re-indent')
...
if (require.main === module) {
    const profile = fs.mkdtempSync(path.join(os.tmpdir(), 'dtab-live-'))   // a throwaway profile; short, since VS Code makes a unix socket in it
